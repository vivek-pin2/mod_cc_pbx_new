...
bool is_black_listed(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, call_details_t *call, const char *blist_num)
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "%s", query);
    switch_safe_free(query);
...
bool bridge_call(switch_core_session_t *session, call_details_t *call, const char *dial_num,char *dsn, switch_mutex_t *mutex)
    ...
    if (!IS_NULL(caller_id) && strlen(caller_id) != 0){
    if(!IS_NULL(call->obd.strp_clr_id) )
    {
   ...
   //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, " caller_id :%s  strp_clr_id:%s, ",caller_id,call->obd.strp_clr_id); 
    ...
    if(!IS_NULL(call->obd.gw_prepend_cid))
    {
    ...
    tmp_str = switch_mprintf("{absolute_codec_string='%s',sip_cid_type=%s,origination_caller_id_name=%s,outbound_caller_from_user=%s,sip_auth_username=%s\
                  ,origination_caller_id_number=%s,originate_timeout=%d}sofia/gateway/gw_%s/%s",call->obd.gw_codec,call_hdr,caller_id,caller_id,caller_id,caller_id,dial_timeout, call->obd.gw_id, dial_num);
...
bool outbound(switch_core_session_t *session, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call, const char *num)
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Outbound total bal:%f  sell_rate: %f ", total_blance, atof(call->obd.sell_rate));
...
bool verify_did(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, did_details_t *did)
   ...
   // switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, " -DID--AVAILABLE--:%s", did_no);
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, " DID CALLBACK  sql: %s  did_bill_type:%s",sql,did->bill_type);
...
bool verify_internal_exten(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, call_details_t *call, const char *num, char *path)
    ...
    switch_safe_free(sql);
    res = call->cg.is_init;
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "CG:%d", res);
...
void ivr(switch_core_session_t *session, uint8_t min, uint8_t max, char *audio_file, const char *result, call_details_t *call_info, char *dsn, switch_mutex_t *mutex)
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Dial_string : %s  agent %s", digit_buffer,result);
    }
...
{  
    ...
    sql = switch_mprintf("SELECT  CASE WHEN SUBSTRING(dst,4, 15) !=SUBSTRING('%s',4, 15) THEN dst WHEN SUBSTRING(forward,4, 15)  !=SUBSTRING('%s',4, 15) THEN forward  END calling_num \
    FROM `pbx_realtime_cdr` WHERE  sip_current_application ='click2call' AND src like concat('%%','%s','%%')  AND SUBSTRING(dst,4, 15) != SUBSTRING(forward,4, 15) ORDER by id DESC LIMIT 1;",caller,caller,call->did.num); 

   /*  sql = switch_mprintf("SELECT concat('+',forward)  FROM `pbx_realtime_cdr` \
    JOIN pbx_contact_list  pcl on pcl.id = %d WHERE dst LIKE  concat('%%',%s,'%%') and forward NOT IN('None','NULL','',pcl.phone_number1) \
    AND src!='%s'  ORDER BY `pbx_realtime_cdr`.`id` DESC LIMIT 1",call->did.dst_id,call->did.num,caller); */

    execute_sql2str(dsn, mutex, sql, src, NELEMS(src));
    //switch_core_session_execute_application(session, "info",NULL);
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "PATCH API  %s caller %s ",src,caller);
...
void handle_queue(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call, int i)
   ...
   if(file_exists(wel_mpt)){
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, " wel prom %d .",file_exists(wel_mpt));
...
void handle_ivr(switch_channel_t *channel, char *dsn, switch_mutex_t *mutex, char *path, char *custom_path, call_details_t *call, struct stack *pt, int ivr)
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, " SQL : %s", sql);
    execute_sql_callback(dsn, mutex, sql, cv_init_callback, &call->cv);
    switch_safe_free(sql);
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "SQL: %zu ", (strlen(call->cv.ivr_dgt) + 1) / 2);
    ...
    if (file_exists(tmp)){
     switch_core_session_execute_application(session,"playback",tmp);
    }
    }
    else
    {
    if(file_exists(call->cv.welcome_pmt)) {
